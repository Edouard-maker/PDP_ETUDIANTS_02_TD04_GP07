...
const char* wifi_ssid = "iPhoneEdouard";
...
const char* client_id = "TD04_GP07";
...
void setup() {
  ...
  if (mqtt_client.connect(client_id, mqtt_user, mqtt_pass)){
    mqtt_client.publish("TD00/temp", String(temp_measure).c_str());
    mqtt_client.publish("TD00/relhum", String(relative_humidity_measure).c_str());
  }
